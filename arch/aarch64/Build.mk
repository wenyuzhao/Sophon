...
export RUSTFLAGS= -C link-arg=-T$(kernel_src)/aarch64.ld
...
arch_kernel:
	@cd $(kernel_src) && cargo build --target $(target) --features device-$(strip $(device)),$(strip $(features))
	@cd $(kernel_src) && cargo objcopy --target $(target) -- --strip-all -O binary $(kernel_img) > /dev/null 2>&1 
