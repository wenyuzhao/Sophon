...
impl FAT {
    pub fn init() -> Result<(), ()> {
        unsafe {
            ...
            println!("MBR disk identifier: 0x{:x}", *(&MBR[0x1B8] as *const u8 as usize as *const u32));
            PARTITION_LBA = *(&MBR[0x1C6] as *const u8 as usize as *const u32);
            println!("FAT partition starts at: 0x{:x}", PARTITION_LBA);
            ...
            println!("FAT type: {}", if bpb.spf16 > 0 { "FAT16" } else { "FAT32" });
        }
        Ok(())
    }
    ...
    pub fn ls_root() {
        unsafe {
            ...
            println!("FAT number of root diretory entries: 0x{:x}", s);
            s <<= 5;
            ...
            while (*dir_ptr).name[0] != 0 {
                ...
                // decode attributes
                let a = dir.attr[0];
                print!("{}", if a &  1 != 0 { 'R' } else { '.' }); // read-only
                print!("{}", if a &  2 != 0 { 'H' } else { '.' }); // hidden
                print!("{}", if a &  4 != 0 { 'S' } else { '.' }); // system
                print!("{}", if a &  8 != 0 { 'L' } else { '.' }); // volume label
                print!("{}", if a & 16 != 0 { 'D' } else { '.' }); // directory
                print!("{}", if a & 32 != 0 { 'A' } else { '.' }); // archive
                print!(" ");
                // staring cluster
                print!("{:7x} ", (dir.ch as u32) << 16 | dir.cl as u32);
                // size
                print!("{:5}     ", dir.size);
                // filename
                dir.attr[0] = 0;
